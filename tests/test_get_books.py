...
from summarize_gutenberg.get_books import (
    fetch_default_books,
    process_books,
    author_check,
    url_check,
...
def test_fetch_default_books_connection_error(mocker):
    mocker.patch(
        "requests.get", side_effect=requests.exceptions.ConnectionError("Connection Error")
    )
...
test_authors_no_list = {"name": "Doe, John"}

test_authors_no_name = [{"mame": "Doe, John"}]
...
book_data = [
    ...
    # No formats
    (
        {"id": 1, "title": "Sample Book", "authors": [{"name": "Doe, John"}]},
        {
            1: {
                "title": "Sample Book",
                "author": "John Doe",
    ...
    # No title or author
    (
        {"id": 1, "formats": {"text/plain; charset=us-ascii": "http://example.com"}},
        {
            1: {
                "title": None,
                "author": "No author found.",
